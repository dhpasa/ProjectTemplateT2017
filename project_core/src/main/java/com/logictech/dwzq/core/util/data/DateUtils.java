...
public class DateUtils extends BaseUtils {
     ...
     *             转换异常，抛出系统异常
     */
    public static String changeDateFormat(String date, String pattern,
            String resultPattern) throws SystemException {
        String result = "";
        try {
            Date dateTmp = null;
    ...
    public static boolean checkIsActiveDate(String date) {
            ...
            if(StringUtils.isNotEmpty(date) && !date.contains(DEFAULT_SEPERATOR)){
                return StringUtils.matches(date, DATE_REG);
            }
            // 先验证基本格式是否正确
     ...
     * @return boolean 比较结果
     */
    private static boolean checkDateCompare(String dateFromStr,
            String dateEndStr, String compareType) {
        if (!checkIsActiveDate(dateFromStr) || !checkIsActiveDate(dateEndStr)) {
            return false;
        } else {
            Date dateFrom = DateUtils.stringToDate(dateFromStr,
                    DateUtils.PATTEN_YMD);
            Date dateEnd = DateUtils.stringToDate(dateEndStr,
                    DateUtils.PATTEN_YMD);
    ...
    public static String getWeekOfDate(Date dt) {
        ...
        int w = getDayWeekOfDate(dt);
        return weekDays[w];
    }
    
    /**
     * 根据日期取得日期在一周中的第几天(周日为0； 周六为6)
     * @param dt
     * @return
     */
    public static int getDayWeekOfDate(Date dt){
        Calendar cal = Calendar.getInstance();
            cal.setTime(dt);
    ...
    */   
    public static int getMonthsOfAge(Date dateBegin,Date dateEnd){
     ...
     * @throws SystemException
     */
        public static String yearsBeforeOrAfterNow(String fm,int spans) throws SystemException {
                String result;
                Calendar date = Calendar.getInstance();
                date.set(Calendar.YEAR, date.get(Calendar.YEAR) + spans);
                Format format = new SimpleDateFormat(fm);
                result = format.format(date.getTime());
                return result;
        }
         ...
         *             转换异常，抛出系统异常
         */
        public static String thirtyDaysBeforeNow(String fm,int spans) throws SystemException {
                String result;
                Calendar date = Calendar.getInstance();
                date.add(Calendar.DAY_OF_MONTH, -30);
                Date temp = date.getTime();
                temp = addDay(temp, spans);
                Format format = new SimpleDateFormat(fm);
         ...
         * @return boolean 比较结果
         */
        public static boolean checkDateCompare(String dateFromStr, String dateEndStr, String compareType, String patten) {
            if (!DateUtils.checkIsActiveDate(dateFromStr) || !DateUtils.checkIsActiveDate(dateEndStr)) {
                return false;
            }
            else {
                Date dateFrom = DateUtils.stringToDate(dateFromStr, patten);
                Date dateEnd = DateUtils.stringToDate(dateEndStr, patten);

                if (DateUtils.COMPARE_TYPE_GT.equals(compareType)) {
                    // 大于
                    if (dateFrom.before(dateEnd)) {
                        return true;
                    }
                }
                else if (DateUtils.COMPARE_TYPE_GTE.equals(compareType)) {
                    // 大于等于
                    if (!dateEnd.before(dateFrom)) {
                        return true;
                    }
         ...
         *             转换异常，抛出系统异常
         */
        public static String WeekBeforeNow(String fm,int spans) throws SystemException {
            String result;
            Calendar date = Calendar.getInstance();
